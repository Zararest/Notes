...
Поскольку `%eltptr` тоже является указателем его надо разыменовать через `i32 0`. Затем мы обращаемся к третьему полю в структуре(нумерация с 0) и в этом поле к четвертому fkloat(с индексом 3).
